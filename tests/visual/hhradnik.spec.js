...
test.describe('Hradník auth and interaction smoke tests', () => {
  ...
  test('main navigation opens the intended screens', async ({ page }) => {
    ...
    await expect(page.locator('#content h1')).toHaveText('Naše hradní výprava')
  ...
  test('account menu opens account and settings panels', async ({ page }) => {
    await prepareVisualSession(page)
    const errors = await assertNoPageErrors(page)
    await page.goto('/')
    ...
    await expect(page.locator('[data-menu="account"]')).toBeVisible()
    await expect(page.locator('[data-menu="settings"]')).toBeVisible()
    await page.locator('[data-menu="account"]').click()
    await expect(page.locator('.redesign-panel.open h1')).toHaveText('Můj účet')
    await page.locator('.redesign-panel-back').click()
    await trigger.click()
    await page.locator('[data-menu="settings"]').click()
    await expect(page.locator('.redesign-panel.open h1')).toHaveText('Nastavení')
