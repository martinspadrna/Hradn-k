...
function addGlobalSearch(header){
 const top=header.querySelector('.top');if(!top)return;let wrap=top.querySelector('.globalSearchWrap');if(wrap)return
 wrap=document.createElement('div');wrap.className='globalSearchWrap';const input=document.createElement('input');input.className='globalSearch';input.placeholder='Hledat památku...';input.autocomplete='off';input.addEventListener('keydown',e=>{if(e.key!=='Enter')return;const value=input.value.trim();const catalogBtn=document.querySelector('.redesign-sidebar .nav button:nth-child(2)');if(catalogBtn)catalogBtn.click();setTimeout(()=>{const s=document.querySelector('#search');if(s){s.value=value;s.dispatchEvent(new Event('input',{bubbles:true}))}},0)});wrap.appendChild(input);top.insertBefore(wrap,top.querySelector('.account')||null)
}
